...
describe('scoreRisk', () => {
  ...
  it('keeps docs-only changes low risk', () => {
    const testImpact = mapImpactedTests(safeDocsPr.changedFiles, safeDocsPr.repositoryFiles);
  ...
  it('flags auth changes without nearby tests as high risk', () => {
    const testImpact = mapImpactedTests(riskyAuthPr.changedFiles, riskyAuthPr.repositoryFiles);
  ...
  it('captures dependency updates and agent-looking broad diffs', () => {
    ...
    const agent = scoreRisk(
      agentLookingPr,
      policy,
      mapImpactedTests(agentLookingPr.changedFiles, agentLookingPr.repositoryFiles),
    );
  ...
  it('does not flag campaign branch as agent-authored', () => {
    ...
    expect(result.riskFindings.map((finding) => finding.code)).not.toContain('agent-authored');
  });
  ...
  it('flags copilot bot author as agent-authored', () => {
    ...
    expect(result.likelyAgentAuthored).toBe(true);
    expect(result.riskFindings.map((finding) => finding.code)).toContain('agent-authored');
  });
  ...
  it('increases risk when tests are deleted', () => {
    ...
    expect(result.riskFindings.map((finding) => finding.code)).toContain('deleted-tests');
  });
  ...
  it('increases risk when skipped tests are introduced', () => {
    ...
    expect(result.riskFindings.map((finding) => finding.code)).toContain('skipped-tests');
  });
});
